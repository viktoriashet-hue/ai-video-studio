"""Batch-transcribe every video in a directory with faster-whisper.

Walks <videos_dir> for common video extensions, runs faster-whisper on each
(local, no external API), writes transcripts to <videos_dir>/edit/transcripts/<name>.json.
...
Usage:
    ...
    python helpers/transcribe_batch.py <videos_dir> --model medium --language ru
...
from transcribe import DEFAULT_COMPUTE_TYPE, DEFAULT_DEVICE, DEFAULT_LANGUAGE, DEFAULT_MODEL, get_model, transcribe_one
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Parallel batch transcription of a videos directory (local faster-whisper)")
    ...
    ap.add_argument("--language", type=str, default=DEFAULT_LANGUAGE, help=f"ISO language code (default: {DEFAULT_LANGUAGE})")
