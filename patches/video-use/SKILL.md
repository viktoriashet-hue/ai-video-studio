...
- `faster-whisper` is installed in the repo's env (`uv sync` / `uv add faster-whisper`) and its model weights are reachable (downloads from Hugging Face Hub on first use, then cached locally — no per-call API key or cost).
...
- **`transcribe.py <video>`** — single-file local faster-whisper call (`--model medium --language ru` by default). No diarization, no API key. Cached.
...
- **Enabling VAD filtering on the local Whisper pass.** It can swallow short filler words ("эм", "ну", "вот") the pipeline needs to find and cut. Keep `vad_filter=False`.
